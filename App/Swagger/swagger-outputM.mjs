...
export default {
  ...
  info: {
    ...
    title: "BIGO API",
    description:
      "Every request should contain a unique tag header that is randomly generated and encrypted on the receiver using JWT with the key 21321f12edqwf23rf2312e12312r12e1. This is the only way to verify the request. The key should be kept exclusively in the receiver, not in the API. Ensure that the encrypted text is randomly generated and unique for each request. And please keep in mind that there will be a message key for every request that will have the message for the receiver user, so it should show if an error status code is present. The idea of the API is that you get the categories, which are the main categories to choose from, and then there are the sub-categories inside, shown on the left, normally. Then there are the items. To watch On click, you create a session, and then you use the session API to return a movie or live or series, to watch it. And each session can be used only once. Use the tag header to verify the request.",
  },
  schemes: ["https", "http"],
  paths: {
    "/Categories": {
